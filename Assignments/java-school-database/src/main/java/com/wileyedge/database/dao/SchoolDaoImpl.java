...
public class SchoolDaoImpl implements SchoolDao {
    ...
    * Do not change any code outside of the placeholders provided.
    ...
    public List<Course> csCourses() {
        ...
        // YOUR CODE STARTS HERE
         ...
         String sql = "SELECT courseCode, courseDesc " +
        	     "FROM course " +
        	     "WHERE teacherId IN (SELECT tid FROM teacher WHERE dept = 'Computer Science')";
    ...
    public List<TeacherCount> teacherCountByDept() {
        ...
        String sql = "SELECT t.dept AS department, COUNT(*) AS teacherCount " +
        		"FROM teacher t " +
        		"GROUP BY t.dept";
    ...
    public List<StudentClassCount> studentsPerClass() {
        ...
        String sql = "SELECT course.courseCode, course.courseDesc, teacher.tFName, teacher.tLName, COUNT(course_student.student_id) AS numStudents " +
        	    "FROM course " +
        	    "INNER JOIN course_student ON course.cid = course_student.course_id " +
        	    "INNER JOIN teacher ON course.teacherId = teacher.tid " +
        	    "GROUP BY course.courseCode, course.courseDesc, teacher.tFName, teacher.tLName";
