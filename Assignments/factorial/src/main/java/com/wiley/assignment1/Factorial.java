...
public class Factorial {
	 ...
	 public int isInputNumberValid()  {
		...
		- All code should be inside the place-holders below.
		*/

		//YOUR CODE STARTS HERE
		 
		 if(scanner.hasNextInt()) { // Checking the user has entered an integer
			 
			 num = scanner.nextInt();
			 
			 if(num >= 1 && num <= 10) { // Checking num is in the desired range
				 
				 return num; // Returning user's number
			 }
			 else {
	 ...
	 private void calculateFactorial(int num) {
		...
		*/
		...
		for(int i = 1; i <=num; i++) {
			...
			result *= i; // Multiplying result by i and updating value with each loop
		}
